...
#					m.bias.data.zero_()
	...
	def forward(self, x, find_max_mem=False, max_mem_layer=0, percentile=90.0):
			...
			for l in range(len(self.pre_process)):
				...
				if isinstance(self.pre_process[l], nn.Conv2d):
					...
					if find_max_mem and l==max_mem_layer:
						cur = self.percentile(self.pre_process[l](out_prev).view(-1), percentile)
						if (cur>max_mem):
					...
					self.mem[l] 	= getattr(self.leak, 'l'+str(l)) *self.mem[l] + self.pre_process[l](out_prev)
					mem_thr 		= (self.mem[l]/getattr(self.threshold, 't'+str(l))) - 1.0
					rst 			= getattr(self.threshold, 't'+str(l)) * (mem_thr>0).float()
